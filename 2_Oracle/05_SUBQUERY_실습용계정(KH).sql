/*
    * 서브쿼리 : 하나의 쿼리문 내에서 사용되는 또 다른 쿼리문
                메인 쿼리문의 조건이나 결과를 위 먼저 실행되어 값을 제공 (보조 역할)
*/
...
-- 전체 직원의 평균 급여보다 더 많은 급여를 맏는 직원 정보를 조회
...
/*
    * 서브쿼리의 종류 *
    ...
    - 다중행 다중열 서브쿼리  : 수행 결과가 여러 행이고, 여러 개의 컬럼일 때 (N행 N열)
...
-- 노옹철 직원보다 급여를 더 많이 맏는 직원 정보 조회(이름,부서코드,급여)
SELECT SALARY
FROM EMPLOYEE
WHERE EMP_NAME = '노옹철';
...
---------------------------------------------------------------------------
/*
    * 다중열 서브쿼리 : 서브쿼리의 겨로가가 한 행이고, 여러 개의 컬럼(열)인 경우
    
    (컬럼1, 컬럼2, ...) = (서브쿼리)
*/
-- 하이유 직원과 같은 부서, 같은 직급에 해당하는 직원 조회(이름, 부서코드, 직급코드, 급여)
...
-- 박나라 직원과 같은 직급이고 같은 사수를 가지고 있는 직원의 정보를 조회(이름 직급코드 사수번호)
...
-- 2) 각 직급별로 최고 급여를 받는 직원 조회 (직원번호, 이름, 직급코드, 급여)
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE (JOB_CODE, SALARY) IN (
                            SELECT JOB_CODE, MAX(SALARY)
...
-- * 보너스 포함 연봉 내림차순 정렬
SELECT EMP_ID, EMP_NAME, (SALARY + (SALARY*NVL(BONUS,0)))*12 "보포 연봉", DEPT_CODE
FROM EMPLOYEE
WHERE (SALARY + (SALARY*NVL(BONUS,0)))*12 >= 30000000
ORDER BY 3 DESC;

-- * 인라인뷰 적용
SELECT EMP_ID, EMP_NAME, "보포 연봉", 부서코드
FROM (
    SELECT EMP_ID, EMP_NAME, (SALARY + (SALARY*NVL(BONUS,0)))*12 "보포 연봉", DEPT_CODE 부서코드
    FROM EMPLOYEE
    ORDER BY 3 DESC
)
WHERE "보포 연봉" >= 30000000;
/*
    * TOP-N 분석 *
        : 상위 N개를 조회
    
    ROWNUM : 조회된 행에 대해서 순서대로 1부터 순번을 부여해주는 가상 컬럼
*/
SELECT EMP_ID, EMP_NAME, "보포 연봉", 부서코드
FROM (
    SELECT EMP_ID, EMP_NAME, (SALARY + (SALARY*NVL(BONUS,0)))*12 "보포 연봉", DEPT_CODE 부서코드
...
/*
    * 순서를 매기는 함수(윈도우 함수, WINDOW FUNCTION)
    ...
    - RANK() OVER(정렬기준)         : 동일한 순위 이후의 등수를 동일한 순위의 개수만큼 건너뛰고 순위 계산
        ...
        1
        4
    - DENSE_RANK() OVER(정렬기준)   : 동일한 순위 이후가 있더라도 그 다음 등수는 +1 해서 순위 계산
        1
        1
        1
        2
...
-- * 해결 : 인라인뷰를 통해 정렬을 한 후 5명을 추려내야 함
...
--      부서별 직원 수를 조회했으나 제대로 조회가 되지 않았다

-- * 문제점 : 부서별 평균 급여가 아닌 각 직원의 급여를 기준으로 조건을 제시했음
