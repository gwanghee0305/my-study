...
SELECT EMP_NAME, LENGTH(EMP_NAME) "직원명(글자수)", LENGTHB(EMP_NAME) "직원명(바이트수)"
        , EMAIL, LENGTH(EMAIL) "이메일(글자수)", LENGTHB(EMAIL) "이메일(글자수)"
FROM EMPLOYEE;
-----------------------------------------------------------------------------
/*
...
SELECT INSTR('AABAACAABBAA', 'B', -1, 2) FROM DUAL; -- 위와 동일
...
ORDER BY EMP_NAME ASC; -- 이름 가나다 순으로 정렬(오름차순)
...
/*
    * LPAD / RPAD : 문자열을 조회할 때 통일감 있게 조회하고자 할 때 사용
                    문자열에 덧붙이고자 하는 문자를 왼쪽 또는 오른쪽에 붙여서 최종 길이만틈 문자열을 만환
                    => 결과는 문자 타입
...
-- LEADING => 왼쪽에 있는 문자열을 제거 LTRIM과 유사
SELECT TRIM(TRAILING 'L' FROM 'LLLLLHLLLLL') FROM DUAL;
-- TRAILING => 오른쪽에 있는 문자열을 제거 RTRIM과 유사
---------------------------------------------------------------------------
...
-- 직원 정보를 조회 (* 출력형식 : [직원번호][직원명]님) -> 200선동일님
...
SELECT TRUNC(123.456, -1) FROM DUAL;    -- 120 일의 자리 버림처리
--------------------------------------------------------------------------
/*
    * 날짜 타입의 데이터 처리 함수 *
*/
...
-- * 직원들의 수습 종료일 조회(이름, 입사입, 입사일+3개월)
SELECT EMP_NAME, HIRE_DATE, ADD_MONTHS(HIRE_DATE, 3) "수습 종료일"
FROM EMPLOYEE;
--------------------------------------------------------------------------
/*
    * NEXT_DAY : 특정 날짜 이후로 지정한 요일의 가장 가까운 날짜를 변환
...
SELECT SYSDATE, NEXT_DAY(SYSDATE, 6) FROM DUAL;     -- 숫자는 언어 상관없이 사용가능

ALTER SESSION SET NLS_LANGUAGE = KOREAN;            -- 시스템 언어 변경
SELECT NEXT_DAY(SYSDATE, '금요일') FROM DUAL;
SELECT NEXT_DAY(SYSDATE, '금') FROM DUAL;

ALTER SESSION SET NLS_LANGUAGE = AMERICAN;          -- 시스템 언어 변경
...
FROM EMPLOYEE
ORDER BY 2, 3, 4;   -- 컬럼 순번을 사용 -> 1 : EMP_NAME / 2 : 입사년도 / 3 : 입사월 / 4 : 입사일
...
-- * 숫자 -> 문자
SELECT 1234 "숫자 타입 데이터", TO_CHAR(1234) "문자타입 데이터" FROM DUAL;
...
SELECT '10000' + '500' FROM DUAL;       -- 10000 + 500

-- 오류 발생
SELECT '10,000' + '500' FROM DUAL; 

SELECT TO_NUMBER('10,000', '99,999') + '500' FROM DUAL;     -- 10500
SELECT TO_NUMBER('10,000', '99,999') + TO_NUMBER('500', '999') FROM DUAL;  -- 10500
-----------------------------------------------------------------------------
/*
...
SELECT TO_DATE('060706') FROM DUAL; -- 060706 -> 60706 으로 인식 => '060706' 으로 수정

-- TO_DATE 기본 포맷 : YYYYMMDD 또는 YYMMDD 일 것
SELECT TO_DATE('260706 143940') FROM DUAL;      -- 오류
SELECT TO_DATE('260706 143940', 'YYMMDD HH24MISS') FROM DUAL; -- 'YYMMDD HH24MISS' 데이터 형식 그대로 작성  
----------------------------------------------------------------------------
/*
    * NULL 처리 함수
*/

-- * NVL : 해당 컬럼의 값이 NULL 인 경우 다른 값으로 대체해주는 함수
--  NVL(컬럼명, 대체할 값)
...
/*
    CASE WHEN THEN : 조건식에 따라 결과값을 반환해주는 구분(함수)
...
/*
    * MIN / MAX : 가장 작은 값 / 가장 큰 값 반환
    ...
    MIN(데이터) / MAX(데이터)
    => 데이터는 모든 타입이 가능함(숫자, 날짜, 문자)
*/
SELECT MIN(EMP_NAME) "문자타입의 최솟값", MIN(SALARY) "숫자타입의 최솟값", MIN(HIRE_DATE) "날짜 타입의 최솟값"
FROM EMPLOYEE;

SELECT MAX(EMP_NAME) "문자타입의 최댓값", MAX(SALARY) "숫자타입의 최댓값", MAX(HIRE_DATE) "날짜 타입의 최댓값"
FROM EMPLOYEE;

/*
    * COUNT : 행의 갯수를 반환(단, 조건이 있을 경우 해당 조건에 맞는 행의 개수 반환)
    
    COUNT(*) : 조회된 결과의 모든 행 갯수를 반환
    COUNT(컬럼) : 해당 컬럼값이 NULL이 아닌 것만 세어서 갯수를 반환
    COUNT(DISTINCT 컬럼) : 해당 컬럼 값의 중복을 제거한 후의 갯수를 반환
                        => 중복 제거 시 NULL은 포함하지 않고 셈함
*/
-- 전체 직원 수 조회
SELECT COUNT(*)
FROM EMPLOYEE;
