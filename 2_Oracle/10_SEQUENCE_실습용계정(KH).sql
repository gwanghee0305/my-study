...
/*
    * 시퀀스 생성 *
    ...
    [NOCACHE | CACHE 숫자]    --> 캐시메모리 할당 여부(기본값 CACHE 20)
                                ...
                                * NOCACHE : 필요할 때마다 그때그때 번호를 생성(결제, 주문번호 등에 사용)
...
-- SEQ_EMPNO 시퀀스 생성
-- * 시작번호: 300, 증가값: 5, 최대값: 310, 순환 X, 캐시메모리 X
...
--> NEXTVAL을 한번도 수행하지 않은 상태에서 CURRVAL을 사용하면 에러 발생
...
--> 오류발생!! 최댓값(310)을 초과할 수 없음 (NOCYCLE 옵션으로 생성했기 때문에 오류남)
------------------------------------------------------------------------------
/*
    * 시퀀스 변경 *
    ...
    => START WITH(시작값)은 변경할 수 없음
        변경하고자 한다면, 제거(DROP)한 후 다시 생성(CREATE)해야함
*/
...
INSERT INTO EMPLOYEE (EMP_ID, EMP_NAME, EMP_NO, JOB_CODE, HIRE_DATE)
            VALUES (SEQ_ENO.NEXTVAL, '임광희', '061002-1000000','J6', SYSDATE);
            --> 300번 직원번호가 저장됨
INSERT INTO EMPLOYEE (EMP_ID, EMP_NAME, EMP_NO, JOB_CODE, HIRE_DATE)
            VALUES (SEQ_ENO.NEXTVAL, '황광희', '051003-1000000','J6', SYSDATE);
             --> 301번 직원번호가 저장됨
SELECT * FROM EMPLOYEE;
ROLLBACK;           --> 300,301번 직원 정보가 지워짐

INSERT INTO EMPLOYEE (EMP_ID, EMP_NAME, EMP_NO, JOB_CODE, HIRE_DATE)
            VALUES (SEQ_ENO.NEXTVAL, '박광희', '041004-1000000','J6', SYSDATE);
            --> 롤백과 상관없이 시퀀스 번호는 증가됨 302번으로 저장
ROLLBACK;
