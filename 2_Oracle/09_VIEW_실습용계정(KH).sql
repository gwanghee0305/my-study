/*
    * VIEW (뷰) *
    ...
    => 자주 사용되는 길고 복잡한 쿼리문을 저장해주면 매번 다시 작성할 필요가 없음
...
-- 러시아에서 근무하는 직원 정보 조회SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME
FROM EMPLOYEE
    JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID
    JOIN LOCATION ON LOCATION_ID = LOCAL_CODE
...
--> 관리자 계정으로 현재 사용자레게 CREATE VIEW 권한 부여
-- GRANT CREATE VIEW TO C##KH;    --> 관리자계정으로 실행
...
--> 뷰를 가지고 SELECT 문을 실행하게 되면, 내부적으로는 인라인 뷰로 변환되어 실행됨
