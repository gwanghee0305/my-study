...
--> DEPARTMENT 테이블에서 D3, D4, D7 부서코드에 해당하는 직원이 EMPLOYEE 테이블에 존재하지 않아 결과에서 제외
...
SELECT EMP_ID, EMP_NAME, JOB_NAME
FROM EMPLOYEE
    JOIN JOB USING (JOB_CODE);  -- USING 사용

SELECT EMP_ID, EMP_NAME, JOB_NAME
FROM EMPLOYEE E
    JOIN JOB J ON E.JOB_CODE = J.JOB_CODE;  -- ON 사용
----------------------------------------------------------------------------
/*
...
--   또는 BETWEEN으로 사용 가능
    JOIN SAL_GRADE ON SALARY BETWEEN MIN_SAL AND MAX_SALL;
------------------------------------------------------------------------------
/*
    * 자체 조인 / 셀프 조인
     : 하나의 테이블을 마치 서로 다른 테이블 처럼 '별칭'을 주어 조인하는 구문
       ( => 이름이 같기 때문에 별칭 필수)
      테이블 내에 자기 자신과 관계가 있는 데이터를 조회할 때 사용
*/
-- 전체 직원의 직원번호, 이름, 부서코드, 사수직원번호, 사수명, 사수 부서코드 조회

-- 직원 정보 => EMPLOYEE / 사수 직원 정보 => EMPLOYEE
SELECT E.EMP_ID 직원번호, E.EMP_NAME 직원명, E.DEPT_CODE "직원 부서코드",
        M.EMP_ID 사수직원번호, M.EMP_NAME 사수명, M.DEPT_CODE "사수 직원코드"
...
WHERE DEPT_CODE = DEPT_ID           -- EMPLOYEE 와 DEPARTMENT 테이블을 조인하기 위한 조건
    AND E.JOB_CODE = J.JOB_CODE    -- EMPLOYEE 와 JOB 테이블을 조인하기 위한 조건
ORDER BY EMP_ID;
