...
--> 접속 시도 시 권한이 부여 되어 있지 않아, CREATE SESSION 권한이 없다고 오류 발생
...
/*
    ...
    * 권한 부여 *
    ...
    예시) TEST 계정에 KH계정의 EMPLOYEE 테이블을 조회할 수 있도록 권한 부여
        GRANT SELECT ON KH.EMPLOYEE TO TEST;               -- Oracle 12c 이전 버전
        
        GRANT SELECT ON C##KH.EMPLOYEE TO C##TEST;         -- Oracle 12c 이후 버전
*/
------------------------------------------------------------------------------
/*
    * 권한 회수 * 
    REVOKE 회수할 권한 FROM 사용자명;            -- 시스템 권한
    REVOKE 종류 ON 객체정보 FROM 사용자명;-- 객체 접근 권한

    예시) TEST 계정에 부여했던 KH계정의 EMPLOYEE 테이블 조회 권한 회수
    REVOKE SELECT ON C##KH.EMOLOYEE FROM C##TEST;
*/
-----------------------------------------------------------------------------
/*
...
WHERE ROLE IN ('CONNECT', 'RECOURCE');
-----------------------------------------------------------------------------
/*
    * TCL (TRANSACTION CONTROL LANGUEGE, 트렌잭션 제어어)
    
    - 트렌잭션 : 데이터베이스의 논리적 연산 단위
    ...
    - 종류
        ...
        * SAVEPOINT 포인트명 (시점저장) : 현재 시점에 변경사항들을 임시로 저장해두는 것을 의미
...
DELETE FROM EMP01 WHERE EMP_ID IN ('217', '214');   -- > 트렌젝션 시작

ROLLBACK;       --> 변경사항 취소 (삭제 전으로 돌아감). 트렌젝션 종료
----------------------------------------------------------------------------
DELETE FROM EMP01 WHERE EMP_ID IN '217';        -- > 트렌젝션 시작
DELETE FROM EMP01 WHERE EMP_ID IN '214'; 

COMMIT;             --> 변경사항 적용. 디스크에 반영됨. 트렌젝션 종료.
---------------------------------------------------------------------------
ROLLBACK;           --> 변경사항 취소 => 마지막 커밋 시점으로 돌아감
SELECT * FROM EMP01 ORDER BY EMP_ID;
--------------------------------------------------------------------------
DELETE FROM EMP01 WHERE EMP_ID IN ('208', '209', '210');    --> 트렌젝션 시작
SELECT * FROM EMP01 ORDER BY EMP_ID;
...
--> DDL(CREATE, ALTER, DROP)을 사용하게 되면 기준 트랜잭션에 저장된 변경사항들은 무조건 반경됨(COMMIT)
-- DDL을 사용하기 전에는 DML(INSERT, UPDATE, DELETE)로 작성된 쿼리문이 있다면
-- 확실하게 트랜잭션 처리(COMMIT, ROLLBACK)한 후에 DDL 을 실행해야 함
