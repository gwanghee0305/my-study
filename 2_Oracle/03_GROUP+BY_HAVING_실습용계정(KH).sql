...
-- * 직급 별 총 직원수, 보너스를 받은 직원수, 급여 총합, 평균급여, 최저급여, 최고급여 조회 (직급코드 오름차순)
...
-- 2. 각 부서별 평균 급여가 300만원 이상인 부서 조회
SELECT DEPT_CODE, FLOOR(AVG(SALARY)) "평균 급여" 
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
ORDER BY 1;     -- SELECT 절의 첫번째 컬럼 표시
...
------------------------------------------------------------------------------
/*
    * 집합 연산자 : 여러 개의 쿼리문을 하나의 쿼리문으로 만들어주는 연산자
    
    - UNION     : 합집합(두 쿼리문의 결과를 더해줌) --> OR와 유사
    - INTERSECT : 교집합(두 쿼리문을 수행한 결과의 중복되는 부분을 추출) --> AND와 유사
...
-- * UNION ALL 사용 / 중복되는 값 그대로 추출(대북혼, 심봉선)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE 
WHERE DEPT_CODE = 'D5'
UNION ALL
...
-- * INTERSECT *
-- 부서코드가 D5인 사원'이거나' 급여가 300만원 초과인 사원들의 정보 조회(직원번호, 이름, 부서코드, 급여)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5' AND SALARY > 3000000;

-- * INTERSECT 사용 / 교집합의 결과을 추출
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5' 
INTERSECT
