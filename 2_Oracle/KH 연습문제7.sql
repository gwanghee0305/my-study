...
*/
-- 원인 1) GENDER 컬럼은 CHECK 제약조건으로 'M' 또는 'F' 값만 입력될 수 있는데 'X' 값이 입력되어 위배 되었다
INSERT INTO TB_EMP (EMP_NO, EMP_NAME, GENDER, DEPT_CD, CONTACT)
...
-- 5. TB_DEPT 테이블에서 10번 부서('인사팀')를 삭제하려고 합니다. 하지만 현재 10번 부서에는 '홍길동' 사원이 소속되어 있어 
--    외래키 무결성 에러가 발생합니다. 에러를 내지 않고 10번 부서를 안전하게 삭제하기 위한 SQL을 작성하세요.
DELETE FROM TB_DEPT WHERE DEPT_CD = 10; --> 외래키로 사용 시 삭제 불가 (RESTRICTED)

-- 1) CASCADE(전체삭제 -> 부모테이블(TB_DEPT) / 자식테이블(TB_EMP))
-- 2) NULL (부모테이블에서만 삭제하고, 자식 테이블에서는 해당 값을 NULL로 변경) *
--      => ON DELETE SET NULL
-- * 외래키 설정 변경 (삭제 -> 생성)
