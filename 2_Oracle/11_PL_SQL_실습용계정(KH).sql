/*
    * PL/SQL : PROCEDURE LANGUEGE EXTENSION TO SQL
    ...
    [구조]
        ...
        [선언부]       : DECLARE 로 시작. 변수나 상수를 선언하고 초기화하는 부분.(생략가능)
        실행부         : BEGIN으로 시작. SQL문 또는 제어문(조건문,반복문) 등의 로직을 작성하는 부분.(필수항목)
        [예외처리부]    : EXCEPTION 으로 시작. 실행 중 예외(오류) 발생 시 해결하기 위한 부분.(생략가능)
*/
...
/*
    * 일반 타입 변수 *
    ...
    변수명 [CONSTANT] 데이터타입 [:= 값];           -- CONSTANT 상수 사용 시

    * 자바와의 차이점
    1) 자바에서는 데이터타입 변수명; 이지만, PL/SQL 에서는 변수명 데이터타입;
    2) 자바에서는 대입 연산자가 = 이지만 PL/SQL 에서는 := 이다.
...
--    ENAME := '임광희';
  ENAME := '&이름';
    ---> 만약 입력 받을 값이 문자타입의 변수에 저장된다면, '&이름' 처럼 작은 따옴표로 감싸주어야 함
...
/*
    * 참조(레퍼런스) 타입 변수
    [ %TYPE ]
    : 특정 테이블의 특정 컬럼 데이터 타입을 그래도 가져와서 변수로 선언
...
-- [Quiz] 직원 번호를 입력받아, 해당 직원의 직원번호, 이름, 직급코드, 급여, 부서명을 출력해보기
-- => 출력 형식(210번 예) : 210 윤은해,J7,2000000, 해외영업1부
-- => 예외처리부 실습 전이므로, 직원번호는 200~222 만 입력
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    JCO EMPLOYEE.JOB_CODE%TYPE;
    SAL EMPLOYEE.SALARY%TYPE;
    DEP DEPARTMENT.DEPT_TITLE%TYPE;
BEGIN
    SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY, DEPT_TITLE
    INTO EID,ENAME,JCO,SAL,DEP
    FROM EMPLOYEE
       JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID
    WHERE EMP_ID = '&직원번호';
      
        DBMS_OUTPUT.PUT_LINE(EID || ', ' || ENAME || ', ' || JCO || ', ' || SAL || ', ' || DEP);
...
/*
    ...
    * 제어문 - 조건문 *
        - 단일 IF문    : IF 조건식 THEN 실행할 내용 END IF;
        -IF/ELSE문    : IF 조건식 THEN 만족할때실행 ELSE 만족하지않을때실행 END IF;
        -IF/ELSIF문   : IF 조건식1 THEN 실행1 ELSIF 조건식2 THEN 실행2 ELSE 실행3 END IF;
...
BEGIN
    ...
    IF BONUS IS NULL THEN DBMS_OUTPUT.PUT_LINE('보너스를 받지 않는 직원입니다.');        -- NULL값은 비교 대상이 될 수 없음
    ELSE DBMS_OUTPUT.PUT_LINE('보너스: ' || BONUS);                                   -- IS NOT NULL / IS NULL 로 사용
    END IF;
END;
/
-------------------------------------------------------------------------------
...
/*
    ...
    * 자주 만나는 예외들 * 
        ...
        - ZERO_DIVIDE   : 0으로 숫자를 나누려고 할 때 발생 (ArithmenticException <-- 자바)
        - DUP_VAL_ON_INDEX : 기본키(PK), UNIQUE 등 칼럼에 중복된 값을 저장하려고 할 때 발생
*/
