...
Flexbox는 1차원(가로 또는 세로 중 한 방향) 레이아웃으로, 콘텐츠 크기에 맞춰 요소들을 한 줄로 정렬하고 간격을 배분할 때 적합하고, 반면 Grid는 2차원(가로와 세로 동시) 레이아웃으로, 행과 열이 교차하는 격자 틀을 먼저 만든 뒤 요소의 위치와 크기를 정밀하게 배치할 때 사용함
